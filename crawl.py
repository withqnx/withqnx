...
#재질    원단/소재/촉감
#내구성  튼튼함/약함, 사용 후 변형
#디자인  구체적 형태/실루엣/패턴/디테일을 짚을 때만 ("넥라인이 예쁘다", "패턴이 독특하다")
         — 근거 없이 "예쁘다"만 있으면 #디자인이 아니라 #첫인상
#퀄리티  만듦새에 대한 평가일 때만 ("고급스럽다", "잘 만들었다", "완성도가 높다")
         — 마감/재질로 쪼개지면 그쪽을 씀. **"만족합니다/좋아요/감사합니다"에는 붙이지 않음**
#가격    비싸다/싸다/가성비
#구성    포켓/세트/부속품
#기획    상품 설명의 약속을 인용하거나 반박할 때만 ("OO하다더니") — 단순 "좋아요"는 제외
#무게감  무거움/가벼움
#사용성  조립/설명 난이도, 사용 편의, 그립감
#첫인상  근거 없이 겉모습이 좋다/별로다 ("예뻐요", "실물이 더 낫다", "사진보다 못하다")
#선물    선물이거나 남을 위해 산 것 ("아버지 생신에", "언니 선물로")
규칙:
- #배송 금지(배송·포장 aspect가 대신함). 배송·포장과 응대·소통에는 태그를 붙이지 않음.
- 무엇이 좋은지 말하지 않은 순수 감정 표현("만족합니다", "좋아요", "감사합니다", "잘 받았습니다")에는
  어떤 태그도 붙이지 않는다. 태그 없는 긍정은 정상이다.
- 위 15개 밖의 태그를 만들지 않는다. 헷갈리면 안 붙인다.
