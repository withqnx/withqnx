...
이 쇼핑몰은 선주문 후생산이라 배송이 수개월 걸리는 것이 정상입니다. 고객도 이를 알고 삽니다.

후기 하나를 segment 여러 개로 나눕니다. 각 segment는 3개 축을 모두 가집니다.

■ 축1 aspect (무엇에 대한 말인가)
- 상품: 제품 자체 — 품질/소재/사이즈/디자인/사용감. **제품에 동봉·부착된 안내문·설명서도 여기**
- 배송·포장: 배송 속도/지연/박스 상태/오배송/누락/포장
- 응대·소통: 상담/문자/답변/연락, **사이트·게시판 기능, 반품·선주문 정책**

■ 축2 sentiment: 긍정 / 부정 / 중립(평가 없음)
...
- 문의: 질문·확인 요청
- 재구매희망: 이미 파는 제품을 더 사고 싶다
- 제품제안: **없는 제품·사양·후속 시즌을 만들어 달라는 요구** ("여름용도 만들어주세요", "무지 흰양말 출시해주세요"). 제안 자체는 감정이 아니므로 sentiment는 중립

■ tags — aspect가 "상품"일 때만. 아래 15개 밖의 태그를 절대 만들지 마세요.
#사이즈  표기 치수/규격 ("M인데 L 같아요") — 옷의 핏감은 제외
#핏      의류 실루엣/기장/어깨라인
#색상    색의 호불호/기대 일치
#마감    봉제선/이음새/마감처리. **스크래치·기스·자국·얼룩·단차·냄새도 여기**
#재질    원단/소재/촉감
#내구성  사용 후 변형·손상. **보풀·털빠짐, 그리고 "스크래치가 잘 생긴다"처럼 반복 발생도 여기**
#디자인  구체적 형태/실루엣/패턴/디테일을 짚을 때만 ("넥라인이 예쁘다")
#퀄리티  만듦새 평가 ("고급스럽다", "잘 만들었다")
#가격    비싸다/싸다/가성비
#구성    포켓/세트/부속품
#기획    상품 설명의 약속을 인용하거나 반박할 때
#무게감  무거움/가벼움
#사용성  조립/설명 난이도, 사용 편의, 그립감. **동봉 안내문이 헷갈린다는 지적도 여기**
#첫인상  **근거를 대지 않은 호평·혹평 전반** ("예뻐요", "실물이 더 낫다", "만족합니다", "기다린 보람이 있다")
#선물    선물이거나 남을 위해 산 것
규칙: #배송 금지. 배송·포장과 응대·소통에는 태그를 붙이지 않음. 헷갈리면 안 붙임.

■ 판정 규칙 (헷갈릴 때 이대로 따르세요)

1) 이 후기가 달린 상품에 대한 말만 분류합니다. 본문에 다른 제품 얘기가 섞여 있어도
   그것으로는 segment를 만들지 마세요.

2) 기다림·수령 사실은 중립입니다. "드디어 왔다", "N개월 만에 받았다", "오래 기다린 보람"은
   선주문 후생산의 정상 과정이므로 {배송·포장, 중립, 없음} 입니다.
   약속 일정 위반, 연락 두절, 주문 순서 역전, 파손·오배송처럼 **실제 불편**이 있을 때만
   {배송·포장, 부정} 입니다.

3) 부속품(안경집·케이스·잉크·필통 등) 미수령은 {배송·포장, 부정} 입니다.
   단 **본품은 받았고 부속품만 기다리는 상태에서 "언제 오나요"를 묻는 것은 {배송·포장, 중립, 문의}** 입니다.
   분할배송을 이해하고 쓴 말투이기 때문입니다.
   "빨리 보내주세요", "몇 달째입니다", "공지도 없네요"처럼 독촉·질책이 섞이면 그때 부정입니다.

4) 교환·반품 vs 문의:
   **본인이 겪은 하자·불일치를 서술했으면 문장이 의문형이어도 `교환·반품`** 입니다
   ("사이즈가 커서 교환 가능한가요?" → 교환·반품).
   하자 서술 없이 정책·재고·배송시점·사용법만 물으면 `문의` 입니다.

5) 교환·반품 vs 양도·거래: 상대가 회사면 교환·반품, 다른 구매자면 양도·거래입니다
   ("~하실 분", "연락주세요", 전화번호·가격 명시 → 양도·거래).
   **제목과 본문이 어긋나 판단이 갈리면 `교환·반품`** 으로 보냅니다.

6) **재구매·추가구매 의사는 가장 강한 긍정입니다.** intent가 `재구매희망`이면
   sentiment는 원칙적으로 `긍정` 입니다. 표현이 담백하거나("다음엔 색안경도 사고 싶네요")
   품절을 아쉬워해도("다른 색도 사려는데 판매가 끝났네요") 제품에 만족했다는 뜻이므로 긍정입니다.
   아쉬움의 대상은 제품이 아니라 판매 종료이므로 부정으로 보내지 마세요.

7) **intent가 `문의`일 때 sentiment는 «불만을 말했는가»로 정합니다.**
   - 중립: 사용법·요령·사양·수령 시점만 묻는 경우
     ("심지는 어느 쪽으로 돌리나요", "종이에 따라 번지는데 원래 그런가요", "케이스는 언제 오나요")
   - 부정: 하자·미수령·지연·응대 불만을 서술한 뒤 묻는 경우
     ("보풀이 심한데 어떻게 쓰나요", "배송완료인데 못 받았습니다", "답변이 없어 다시 묻습니다")
   질문했다는 사실만으로 부정을 주지 마세요. **판단 기준은 문장 끝이 아니라 불만 서술의 유무입니다.**

8) confidence 는 **개별 축의 판정이 흔들릴 때만** 낮춥니다.
   **segment 를 2개 이상으로 나눈 것 자체는 감점 사유가 아닙니다.** 각 축이 본문에
   명시돼 있으면 여러 개로 나눴어도 0.9 이상을 주세요.
   0.9~1.0 명확 / 0.8~0.9 약간의 여지 / 0.8 미만 정말 애매할 때만.

■ summary는 반드시 채우세요(빈 문자열 금지).
